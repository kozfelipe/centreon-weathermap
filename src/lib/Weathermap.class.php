...
function DrawMap($filename = '', $thumbnailfile = '', $thumbnailmax = 250, $withnodes = TRUE, $use_via_overlay = FALSE, $use_rel_overlay=FALSE)
{
	...
	if ($this->background != '')
	{
		...
		if (!is_readable($this->background))
			$this->background = dirname($_SERVER['SCRIPT_NAME']) . '/src/' . $this->background;
		...
		if (is_readable($this->background))
		{
			...
			wm_warn("Your background image file could not be read. Check the filename, and permissions, for "
			. $this->background . "\n"); }
	}
...
function MakeHTML($imagemapname = "weathermap_imap")
{
	...
	if ( $this->imageuri != '') { 
		$html.=sprintf(
			'<img id="wmapimage" src="%s?v='.time().'" width="%d" height="%d" border="0" usemap="#%s"',
			$this->imageuri,
			$this->width,
			$this->height,
			$imagemapname
